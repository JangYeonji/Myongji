...
public class MyTetris2_9 extends JFrame{
	public MyTetris2_9() {
		setTitle("나의 테트리스");
	...
	class MyPanel extends JPanel{
		...
		public MyPanel() {
			...
			for(int i=0;i<shapeCoord.length;i++) { 
				shapeCoord[i] = new Point(SHAPE_COORD[type][i].x,SHAPE_COORD[type][i].y); //@중요@배열 객체 요소 복사(주소복사, SHAPE_COORD[i].x)
				...
				while(true) {
					...
					if(fallen != true) { //떨어지지 않았다면 //깃발 체크
						...
						for(int i=0;i<shape.length;i++) { 
							...
							int type = (int)(Math.random()*7);
							shapeCoord[i].x = SHAPE_COORD[type][i].x; //새로운 좌표(회전되지 않은)값 복사
		...
		class MyListener extends KeyAdapter{
			...
			public void keyPressed(KeyEvent e) {
				...
				if(e.getKeyCode()==KeyEvent.VK_DOWN) { //오른쪽
					boolean down = false;
					while(!down) {
						down = isFallen();
						if(down!=true) {
		...
		public boolean isBlockNext(Point p){ //Point 타입이라
			...
			for(Point r:rBlock) { //for(int i=0;i<rBlock.size();i++) {
				...
				if(p.x ==r.x && p.y+1 ==r.y){ //if(p.x==rBlock.get(i).x&&p.y+1==rBlock.get(i).y) { 
					result = true;
					break;
				}
			}
