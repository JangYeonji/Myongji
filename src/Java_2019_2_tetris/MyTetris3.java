...
public class MyTetris3 extends JFrame{
	public MyTetris3() {
		setTitle("Inbo Tetris");
...
class MyPanel extends JPanel{
	...
	int score=0;
	MyPanel(){   //생성자
		this.setBackground(new Color(191,216,239));
		setLayout(null);
		title = new JLabel("         iNBo TeTRiS ");
		title.setFont(new Font("bauhaus 93", NORMAL, 30));
		//title.setHorizontalAlignment(SwingConstants.CENTER);
		title.setForeground(new Color(191,216,239));
		title.setBackground(new Color(199,164,153));   //204,190,152
		title.setBounds(0,0,330,80);
		title.setOpaque(true);
		this.add(title);
		int type=(int)(Math.random()*6);
		for(int i=0; i<shapeCoord.length; i++) {
			shapeCoord[i] = new Point(SHAPE_COORD[type][i].x, SHAPE_COORD[type][i].y);
			shape[i] = new Point(SHAPE_COORD[type][i].x+4, SHAPE_COORD[type][i].y+3);
		}// for
...
*/					fallen = isFallen();   // 메소드로 처리
				if( fallen != true) {
					...
					for(int i=0; i<shape.length; i++) {   // rBlock 추가  ArrayList=> size(), add(객체), remove(), get(i)
						if(shape[i].y == 3) {  //****게임끝******//
							title.setBounds(0,270,330,110);
							title.setBackground(new Color(167,114,97));
							title.setText("   GAME OVER! " + "score " + score);
							return; 
						} // 게임끝
					...
					for(int i=0; i<shape.length; i++) {  	
						if( isLineFull(shape[i].y) == true ) { // 해당 블록이 최종 놓인 라인이 가득차면
							...
							score+=8;  //****스코어*****//
							title.setText("   Score "+Integer.toString(score)); //****출력*****//
						}
					}//for 중요
	...
	public boolean isFallen(){
		...
		for(int i=0; i<shape.length; i++) {
			if(shape[i].y == 16 || isBlockNext(shape[i]) ) {
				result = true;
				break;  // 가장 가까운 루프를 벗어남
			}//if
		}//for
...
/*			for(Point r : rBlock) {
	...
	protected void paintComponent(Graphics g) {
		...
		for(int i=0; i<shape.length; i++) { //떨어지는 블럭들(shape:배열)
			...
			/*g.setColor(getColor()); 
			g.fillRoundRect(shape[i].x*40-40+3, shape[i].y*40-40+3, 34, 34, 8, 8);*/
		}
		g.setColor(new Color(199,164,153));  
